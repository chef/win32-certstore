...
describe Win32::Certstore do
  ...
  describe "#list_cert" do
    ...
    context "When passing valid certificate store name" do
      ...
      before(:each) do
        allow_any_instance_of(Win32::Certstore::Certificate).to receive(:list_cert).and_return([root_certificate_name])
      end
    ...
    context "When passing valid certificate store name" do
      let (:store_name) { "root" }
      before(:each) do
        allow_any_instance_of(Win32::Certstore::Certificate).to receive(:list_cert).and_return([])
      end
